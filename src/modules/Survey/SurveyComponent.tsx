...
export default function SurveyComponent(props: SurveyComponentProps) {
    ...
    const saveToDatabase = async () => {
        if (firebase.firestore() && props.dbCollectionName) {
            const document = await firebase
                .firestore()
                .collection(props.dbCollectionName)
                .add({
                    name: contacts.name,
                    email: contacts.email,
                    time: Date.now(),
                });
            console.log(props.dbCollectionName);

            const batch = firebase.firestore().batch();
            await saveToDatabaseDocument(
                document,
                "questionResults",
                questionResults,
                batch
    ...
    const saveToDatabaseDocument = async (
        ...
        questionResults.forEach((questionResult, i) => {
            const questionResultsCollection = document.collection(
                collectionName
            );
            //@ts-ignore
            Object.keys(questionResult).forEach(
                //@ts-ignore
                (key) =>
                    //@ts-ignore
                    questionResult[key] === undefined &&
                    //@ts-ignore
                    delete questionResult[key]
            );
            const doc = questionResultsCollection.doc(
    ...
    const submitErrorDialog = (
        ...
        >
            ...
            <DialogActions>
                <Button
                    onClick={() => setSubmitErrorDialogState(false)}
                    color="primary"
                />
            </DialogActions>
        </Dialog>
